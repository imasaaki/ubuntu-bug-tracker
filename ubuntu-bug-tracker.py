...
page_1804_true_set = set()
page_1804_false_set = set()

def add_set(a_set, a_url):
    if a_url in a_set:
...
def getBugList(a_url, task):
    ...
    with open("output_" + task + datetime.datetime.now().strftime('%Y%m%d_%H%M%S') + '.csv', mode='a') as out_file:
        out_file.write('"url","module","importance","status","lastupdate","milestone","1804"' + '\r\n')
        for i in range(int(total_number/single_pange_number) + 1):
            matches_url = re.findall(pattern_url, noreturn_text)
            matches_importance = re.findall(pattern_importance, noreturn_text)
            matches_status = re.findall(pattern_status, noreturn_text)
            matches_lastupdate = re.findall(pattern_lastupdate, noreturn_text)
            ...
            try:
                for j in range(len(matches_url)):
                    ...
                    if dt_bug > dt_1804_release:
                        # 一度見たものはもう見ない
                        if matches_url[j][0] in page_1804_true_set:
                            is_ubuntu1804 = True
                            print('hit true cache: ' + matches_url[j][0])
                        elif matches_url[j][0] in page_1804_false_set:
                            is_ubuntu1804 = False
                            ...
                            if '18.04' in response_bugpage.text:
                                is_ubuntu1804 = True
                                add_set(page_1804_true_set, matches_url[j][0])
                            else:
                                add_set(page_1804_false_set, matches_url[j][0])
            ...
            response.encoding = response.apparent_encoding
            noreturn_text = response.text.replace('\n', '').replace('\r', '').replace(' ', '')
            next_page = (i) * single_pange_number
